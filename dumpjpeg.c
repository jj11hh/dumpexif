...
int main (int argc, char * argv[]) {
    ...
    uint16_t tag = 0;
    if (argc != 3){
        fprintf(stderr, "usage: %s IMAGE.jpg IMAGE.exif\n", argv[0]);
        return -1;
    }
    ...
    FILE* metafile = fopen(argv[2], "w");
    if (! jpegfile){
        perror("Cannot open matedata file");
        return -1;
    }
    
    uint16_t soi = htons(tag);
    fwrite(&soi, sizeof(uint16_t), 1, metafile);
    ...
    while (! feof(jpegfile)) {
        ...
        if ((tag & 0xFFF0) == 0xFFE0) { // is a APPx marker
            ...
            fwrite((const void *)seg, sizeof(char), len, metafile);
        }
        else {
